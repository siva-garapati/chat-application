...
const SideBar1 = () => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:w-24 lg:w-34 md:flex md:flex-col bg-white border-r border-gray-200 pt-6">
          ...
          {icons.map((item) => {
            return (
              <Link
                ...
                to={item.path}
                title={item.title}
                state={item.path==='/new-message' && {backgroundLocation:location}}
                className={`p-3 rounded-xl transition-all duration-200 
                  ${isActive(item.path) ? "bg-black text-white" : "hover:bg-gray-100 text-gray-600"}`}
              >
          ...
          {icons.map((item) => {
            return (
              <Link
                ...
                ${isActive(item.path) ? "text-[rgb(29,155,240)]" : "text-gray-600 hover:text-[rgb(29,155,240)]"}`}
              >
                <item.icon size={24} />
